...
export interface FileDropzoneProps {
  ...
  children?: ReactNode
  disabled?: boolean
  /** Shows a spinner + "Memproses…" in place of the upload prompt, and
   * blocks further clicks/drops — set while the caller is awaiting a
   * long-running upload (e.g. AI extraction) so it's obvious a new file
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      <div
        ...
        className={cn(
          'flex flex-col items-center justify-center gap-2 rounded-card border-2 border-dashed p-8 text-center transition-colors duration-150',
          loading ? 'cursor-wait' : 'cursor-pointer',
          'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2',
          loading && 'border-primary bg-primary/5',
          !loading && dragging && 'border-primary bg-primary/5 text-primary',
          !loading && !dragging && 'border-line bg-surface hover:border-primary/40 hover:bg-primary/5 text-fg-muted',
      ...
      >
        {loading ? (
          <>
            ...
            <p className="text-body font-medium text-primary" role="status">
              {loadingLabel}
            </p>
            <p className="text-caption text-fg-subtle mt-0.5">Tunggu sampai selesai sebelum unggah lagi</p>
          </>
        ) : (
          <>
            <UploadCloud
